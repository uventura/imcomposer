...
ImComposer::Core::Parser::JsonWidgetReader::JsonWidgetReader(
...
void ImComposer::Core::Parser::JsonWidgetReader::beginElement(
    const json& element) {
  ...
  if (ImGui::IsItemClicked() ||
    ...
    drawList->AddRect(
        ImVec2(initialPosition.x - spacement, initialPosition.y - spacement),
        ImVec2(initialPosition.x + itemDimensions.x + spacement,
               initialPosition.y + itemDimensions.y + spacement),
        col);
  }
}

void ImComposer::Core::Parser::JsonWidgetReader::endElement(
    const json& element) {}